...
import { getAssociatedTokenAddress, getAccount, TOKEN_PROGRAM_ID } from '@solana/spl-token-37';
import { IDL as SoulBoundIdl, SoulBoundAuthority } from './_idls/soulBoundAuthority';
import { IDL as CardinalStakePoolIdl, CardinalStakePool } from './_idls/cardinalStakePool';
import { IDL as CardinalRewardDistributorIdl, CardinalRewardDistributor } from './_idls/cardinalRewardDistributor';

const BN = anchor.BN;

export function createStakeApi(PROVIDER: anchor.AnchorProvider) {
    //
    ...
    //
    const AUTHORIZATION_RULES_PROGRAM_ID = new PublicKey('auth9SigNpDKz4sJJ1DfCTuZrZNSAgh9sFD3rboVmgg');
    const AUTHORIZATION_RULES = new PublicKey('eBJLFYPxJmMGKuFwpDWkzxZeUrad92kZRC5BJLpzyT9');

    //
    // Stake program clients.
    //
    const SOUL_BOUND_PROGRAM = new Program<SoulBoundAuthority>(SoulBoundIdl, SOUL_BOUND_PROGRAM_ID, PROVIDER);
    ...
    const fetchRewardEntry = async ({
        ...
        const rewardEntryAccount = await rewardDistributorProgram.account.rewardEntry.fetch(rewardEntry);
    ...
    async function claimRewardInstruction({
        ...
        const userRewardMintTokenAccount = await getAssociatedTokenAddress(goldMint, scopedSbaUserAuthority, true);
    ...
    async function isSoulBoundAuthorityUserInitialized(user: PublicKey, soulboundProgram = SOUL_BOUND_PROGRAM) {
        const [sbaUser] = PublicKey.findProgramAddressSync(
            [Buffer.from('sba-scoped-user'), user.toBuffer()],
            soulboundProgram.programId
        );
    ...
    async function readUnclaimedGoldPoints({
        ...
        const rewardDistributorAcc = accounts
            ...
            : await rewardDistributorProgram.account.rewardDistributor.fetch(rewardDistributor);
        ...
        const claimedAmount = await (async () => {
            try {
                const rewardTokenAccount = accounts
                    ...
                    : await getAccount(soulboundProgram.provider.connection, userRewardMintTokenAccount);
    ...
    async function transferRewards({
        ...
        const fromRewardEntry = PublicKey.findProgramAddressSync(
            [Buffer.from('reward-entry'), rewardDistributor.toBuffer(), fromStakeEntry.toBuffer()],
            rewardDistributorProgram.programId
        )[0];
        ...
        const toRewardEntry = PublicKey.findProgramAddressSync(
            [Buffer.from('reward-entry'), rewardDistributor.toBuffer(), toStakeEntry.toBuffer()],
            rewardDistributorProgram.programId
        )[0];
        const toScopedSbaUserAuthorityAta = await getAssociatedTokenAddress(goldMint, toScopedSbaUserAuthority, true);

        const fromNftToken = await getAssociatedTokenAddress(fromNft.mintAddress, fromUser);
